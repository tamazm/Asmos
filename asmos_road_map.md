...
Popup builder UI (drag-and-drop or template-based v1; templates first, full builder
...
Widget renders popup + spin-the-wheel game + form (name/email/phone capture)
...
Extend widget to capture: clicks, scroll depth, time-on-page, navigation, exit intent, sampled mouse movement
...
Queue setup (Redpanda or Kafka; start with Redpanda for simpler operations)




Stream consumer service: writes raw events + computes session-level aggregates ClickHouse instance for event/behavioral storage
...
Fixed-split traffic allocation (simple % split across variants)
...
## Goal: real differentiation through adaptive optimization within a single customer's data
...
Auto-reallocation of traffic toward winning variants (replacing fixed-split allocation)
...
Exit criteria: popups optimize themselves in near real-time per customer, without a human declaring a winner.

### Phase 5: Industry Data Corpus & Cross-Customer Priors 

Goal: solve the cold-start problem for low-traffic customers
...
Aggregate anonymized performance patterns by industry from your own customer base Bandit prior initialization: new customers start from industry-informed priors instead of zero data




Legal review: ensure cross-customer aggregation is anonymized/compliant, not raw PII sharing

Exit criteria: a brand-new low-traffic customer's popups perform reasonably well from day one, not after weeks of blind testing.

### Phase 6: LLM Generative Agent Layer 

Goal: the ambitious version, with AI that creates and explains instead of only allocating traffic
...
Customer-facing "agent" UI (chat or report format for insights)

Exit criteria: the full loop is closed. The agent generates variants, the bandit tests them, the agent explains results, and the cycle repeats autonomously.
...
Cost monitoring (ClickHouse storage and LLM API costs scale with usage; watch this from Phase 2 onward)



Customer feedback loop: talk to pilot customers before building Phase 4+ (validate whether they want auto-optimization or only good reporting)
